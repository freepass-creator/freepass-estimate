...
const 자동전진단계 = computed(() => currentStep.value.key === 'vehicle'
  && ['brand', 'model', 'variant', 'trim'].includes(vehicleState.subStep || 'brand'));
...
<template>
  <div class="m-shell">
    ...
    <footer
      class="m-footer ui-bottom-action"
    ...
    >
      ...
      <template v-else>
        <button v-if="하단내비.hasPrev" class="m-btn m-btn--ghost ui-button secondary" :class="{ 'm-btn--icon': 견적보기보임 }"
